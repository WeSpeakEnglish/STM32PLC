...
void TM_DMA2DGRAPHIC_Fill(uint32_t color) {
  ...
  hdma2d.Init.OutputOffset =  DIS.Offset;
	

 /*##-3- Start DMA2D transfer ###############################################*/  
  HAL_DMA2D_Start_IT(&hdma2d, color, DIS.StartAddress,  (uint32_t)DIS.Width, (uint32_t)DIS.Height);
...
void TM_DMA2DGRAPHIC_CopyBufferIT(void* pSrc, void* pDst, uint32_t xSize, uint32_t ySize, uint32_t OffLineSrc, uint32_t OffLineDst) {
	...
	/* Set up pixel format */  
	DMA2D->FGPFCCR = 0;//0LTDC_Pixelformat_RGB565;  ARGB8888
